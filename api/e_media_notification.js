...
router.get('/:id', function(req, res) {
	...
	const id_e_media_notification = parseInt(req.params.id);
	...
	const where = {id: id_e_media_notification};
	for (const field in req.query)
		if (field.indexOf('fk_id_') == 0 || field.indexOf('f_') == 0 && attributes[field])
			where[field] = req.query[field];
	query.where = where;
...
router.get('/:id/:association', function(req, res) {
	...
	const id_e_media_notification = req.params.id;
...
router.put('/:id', function(req, res) {
	...
	models.E_media_notification.findOne({where: {id: id_e_media_notification}}).then(function(e_media_notification) {
		...
		e_media_notification.update(updateObject, {where: {id: id_e_media_notification}}, {user: req.user}).then(function() {
			answer["e_media_notification".substring(2)] = e_media_notification;
