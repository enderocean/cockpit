...
router.get('/:id', function(req, res) {
	...
	const id_e_translation = parseInt(req.params.id);
	...
	const where = {id: id_e_translation};
	for (const field in req.query)
		if (field.indexOf('fk_id_') == 0 || field.indexOf('f_') == 0 && attributes[field])
			where[field] = req.query[field];
	query.where = where;
...
router.get('/:id/:association', function(req, res) {
	...
	const id_e_translation = req.params.id;
...
router.put('/:id', function(req, res) {
	...
	models.E_translation.findOne({where: {id: id_e_translation}}).then(function(e_translation) {
		...
		e_translation.update(updateObject, {where: {id: id_e_translation}}, {user: req.user}).then(function() {
			answer["e_translation".substring(2)] = e_translation;
