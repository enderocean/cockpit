...
const attributes = require('../models/attributes/e_documents_task');
const options = require('../models/options/e_documents_task');
...
router.get('/list', block_access.actionAccessMiddleware("documents_task", "read"), (req, res) => {
	res.render('e_documents_task/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("documents_task", "read"), (req, res) => {
	filterDataTable("E_documents_task", req.body).then(rawData => {
		entity_helper.prepareDatalistResult('e_documents_task', rawData, req.session.lang_user).then(preparedData => {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("documents_task", "read"), (req, res) => {
	...
	models.E_documents_task.findOne({
		where: {
			id: parseInt(sourceId)
		},
		include: include
	}).then(e_documents_task => {
		if (!e_documents_task['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
		...
		e_documents_task['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]({where: include.where, include: include.include}).then(count => {
			const rawData = {
				recordsTotal: count,
				recordsFiltered: count,
				data: []
			};
			for (let i = 0; i < e_documents_task[subentityAlias].length; i++)
				rawData.data.push(e_documents_task[subentityAlias][i].get({plain: true}));
...
router.get('/show', block_access.actionAccessMiddleware("documents_task", "read"), (req, res) => {
	const id_e_documents_task = req.query.id;
	const tab = req.query.tab;
	const data = {
		tab: tab,
		enum_radio: enums_radios.translated("e_documents_task", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_documents_task', id_e_documents_task, options).then(e_documents_task => {
		if (!e_documents_task)
			return res.render('common/error', {error: 404});

		/* Update local e_documents_task data before show */
		data.e_documents_task = e_documents_task;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_documents_task, "e_documents_task").then(_ => {
			status_helper.translate(e_documents_task, attributes, req.session.lang_user);
			data.componentAddressConfig = component_helper.address.getMapsConfigIfComponentAddressExists("e_documents_task");
			enums_radios.translateUsingField(e_documents_task, options, data.enum_radio);
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(data, options).then(data => {
				res.render('e_documents_task/show', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_documents_task");
			})
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_documents_task");
		});
...
router.get('/create_form', block_access.actionAccessMiddleware("documents_task", "create"), (req, res) => {
	const data = {
		enum_radio: enums_radios.translated("e_documents_task", req.session.lang_user, options)
	};
	...
	// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
	entity_helper.getLoadOnStartData(data, options).then(data => {
		const view = req.query.ajax ? 'e_documents_task/create_fields' : 'e_documents_task/create';
		res.render(view, data);
	}).catch(err => {
		entity_helper.error(err, req, res, '/documents_task/create_form', "e_documents_task");
	})
});

router.post('/create', block_access.actionAccessMiddleware("documents_task", "create"), (req, res) => {

	const createObject = model_builder.buildForRoute(attributes, options, req.body);

	models.E_documents_task.create(createObject, {user: req.user}).then(e_documents_task => {
		let redirect = '/documents_task/show?id=' + e_documents_task.id;
		req.session.toastr = [{
			...
			promises.push(new Promise((resolve, reject) => {
				models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
					...
					if (typeof association['add' + modelName] !== 'undefined') {
						association['add' + modelName](e_documents_task.id).then(_ => {
						...
						const obj = {};
						obj[req.body.associationForeignKey] = e_documents_task.id;
		...
		model_builder.setAssocationManyValues(e_documents_task, req.body, createObject, options).then(_ => {
			Promise.all(promises).then(_ => {
				component_helper.address.setAddressIfComponentExists(e_documents_task, options, req.body).then(_ => {
					status_helper.setInitialStatus(req.user, e_documents_task, 'E_documents_task', attributes).then((statusToastrs = []) => {
...
router.get('/update_form', block_access.actionAccessMiddleware("documents_task", "update"), (req, res) => {
	const id_e_documents_task = req.query.id;
	const data = {
		enum_radio: enums_radios.translated("e_documents_task", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_documents_task', id_e_documents_task, options).then(e_documents_task => {
		...
		e_documents_task.dataValues.enum_radio = data.enum_radio;
		enums_radios.translateUsingField(e_documents_task, options, data.enum_radio);
		data.e_documents_task = e_documents_task;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_documents_task, "e_documents_task", false).then(_ => {
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(req.query.ajax ? e_documents_task.dataValues : data, options).then(data => {
				if (req.query.ajax) {
					e_documents_task.dataValues = data;
					res.render('e_documents_task/update_fields', e_documents_task.get({
						plain: true
					}));
				} else
					res.render('e_documents_task/update', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_documents_task");
			})
		}).catch(err => {
...
router.post('/update', block_access.actionAccessMiddleware("documents_task", "update"), (req, res) => {
	const id_e_documents_task = parseInt(req.body.id);
	const updateObject = model_builder.buildForRoute(attributes, options, req.body);

	models.E_documents_task.findOne({
		where: {
			id: id_e_documents_task
		}
	}).then(e_documents_task => {
		if (!e_documents_task)
			return res.render('common/error', {error: 404});

		component_helper.address.updateAddressIfComponentExists(e_documents_task, options, req.body);

		updateObject.version = e_documents_task.version;
		if(typeof e_documents_task.version === 'undefined' || !e_documents_task.version)
			updateObject.version = 0;
		updateObject.version++;

		e_documents_task.update(updateObject, {user: req.user}).then(_ => {

			// We have to find value in req.body that are linked to an hasMany or belongsToMany association
			// because those values are not updated for now
			model_builder.setAssocationManyValues(e_documents_task, req.body, updateObject, options).then(_ => {

				let redirect = '/documents_task/show?id=' + id_e_documents_task;
				if (typeof req.body.associationFlag !== 'undefined')
					redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
				...
				entity_helper.error(err, req, res, '/documents_task/update_form?id=' + id_e_documents_task, "e_documents_task");
			});
		}).catch(err => {
			entity_helper.error(err, req, res, '/documents_task/update_form?id=' + id_e_documents_task, "e_documents_task");
		});
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('documents_task', 'read'), (req, res) => {
	...
	(async () => {
		...
		let e_documents_task;
		switch (option.structureType) {
			case 'hasOne':
				e_documents_task = await models.E_documents_task.findOne({
					where: {
						id: req.params.id
					},
					include: {
				...
				dustData = e_documents_task[option.as];
...
router.get('/set_status/:id_documents_task/:status/:id_new_status', block_access.actionAccessMiddleware("documents_task", "read"), block_access.statusGroupAccess, (req, res) => {
	status_helper.setStatus('e_documents_task', req.params.id_documents_task, req.params.status, req.params.id_new_status, req.user, req.query.comment).then(_ => {
...
router.post('/search', block_access.actionAccessMiddleware('documents_task', 'read'), (req, res) => {
	...
	models.E_documents_task.findAndCountAll(query).then(results => {
					...
					if(fieldSelect == field && results.rows[i][field] && results.rows[i][field] != "")
						switch(attributes[field].newmipsType) {
							...
							case "datetime":
								...
								break;
							case "enum":
								results.rows[i][field] = enums_radios.translateFieldValue('e_documents_task', field, results.rows[i][field], req.session.lang_user)
								break;
							default:
								break;
						}
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("documents_task", "update"), (req, res) => {
	const alias = req.params.alias;
	...
	models.E_documents_task.findOne({
		...
		e_documents_task['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(_ => {
			if(globalConfig.env == "tablet"){
				let target = "";
				for (let i = 0; i < options.length; i++)
					if (options[i].as == alias)
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("documents_task", "create"), (req, res) => {
	const alias = req.params.alias;
	const idEntity = req.body.idEntity;
	models.E_documents_task.findOne({
		where: {
		...
		if (typeof(toAdd = req.body.ids) === 'undefined') {
			...
			return res.redirect('/documents_task/show?id=' + idEntity + "#" + alias);
		}

		e_documents_task['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(_ => {
			res.redirect('/documents_task/show?id=' + idEntity + "#" + alias);
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_documents_task");
		});
	}).catch(err => {
...
router.post('/delete', block_access.actionAccessMiddleware("documents_task", "delete"), (req, res) => {
	const id_e_documents_task = parseInt(req.body.id);
	...
	models.E_documents_task.findOne({
		...
		deleteObject.destroy().then(_ => {
			...
			let redirect = '/documents_task/list';
			if (typeof req.body.associationFlag !== 'undefined')
				redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
			res.redirect(redirect);
			entity_helper.removeFiles("e_documents_task", deleteObject, attributes);
		}).catch(err => {
			entity_helper.error(err, req, res, '/documents_task/list', "e_documents_task");
		});
	}).catch(err => {
		entity_helper.error(err, req, res, '/documents_task/list', "e_documents_task");
	});
