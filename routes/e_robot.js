...
const attributes = require('../models/attributes/e_robot');
const options = require('../models/options/e_robot');
...
router.get('/list', block_access.actionAccessMiddleware("robot", "read"), (req, res) => {
	res.render('e_robot/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("robot", "read"), (req, res) => {
	filterDataTable("E_robot", req.body).then(rawData => {
		entity_helper.prepareDatalistResult('e_robot', rawData, req.session.lang_user).then(preparedData => {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("robot", "read"), (req, res) => {
	...
	models.E_robot.findOne({
		where: {
			id: parseInt(sourceId)
		},
		include: include
	}).then(e_robot => {
		if (!e_robot['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
		...
		e_robot['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]({where: include.where, include: include.include}).then(count => {
			const rawData = {
				recordsTotal: count,
				recordsFiltered: count,
				data: []
			};
			for (let i = 0; i < e_robot[subentityAlias].length; i++)
				rawData.data.push(e_robot[subentityAlias][i].get({plain: true}));
...
router.get('/show', block_access.actionAccessMiddleware("robot", "read"), (req, res) => {
	const id_e_robot = req.query.id;
	const tab = req.query.tab;
	const data = {
		tab: tab,
		enum_radio: enums_radios.translated("e_robot", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_robot', id_e_robot, options).then(e_robot => {
		if (!e_robot)
			return res.render('common/error', {error: 404});

		/* Update local e_robot data before show */
		data.e_robot = e_robot;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_robot, "e_robot").then(_ => {
			status_helper.translate(e_robot, attributes, req.session.lang_user);
			data.componentAddressConfig = component_helper.address.getMapsConfigIfComponentAddressExists("e_robot");
			enums_radios.translateUsingField(e_robot, options, data.enum_radio);
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(data, options).then(data => {
				res.render('e_robot/show', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_robot");
			})
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_robot");
		});
...
router.get('/create_form', block_access.actionAccessMiddleware("robot", "create"), (req, res) => {
	const data = {
		enum_radio: enums_radios.translated("e_robot", req.session.lang_user, options)
	};
	...
	// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
	entity_helper.getLoadOnStartData(data, options).then(data => {
		const view = req.query.ajax ? 'e_robot/create_fields' : 'e_robot/create';
		res.render(view, data);
	}).catch(err => {
		entity_helper.error(err, req, res, '/robot/create_form', "e_robot");
	})
});

router.post('/create', block_access.actionAccessMiddleware("robot", "create"), (req, res) => {

	const createObject = model_builder.buildForRoute(attributes, options, req.body);

	models.E_robot.create(createObject, {user: req.user}).then(e_robot => {
		let redirect = '/robot/show?id=' + e_robot.id;
		req.session.toastr = [{
			...
			promises.push(new Promise((resolve, reject) => {
				models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
					...
					if (typeof association['add' + modelName] !== 'undefined') {
						association['add' + modelName](e_robot.id).then(_ => {
						...
						const obj = {};
						obj[req.body.associationForeignKey] = e_robot.id;
		...
		model_builder.setAssocationManyValues(e_robot, req.body, createObject, options).then(_ => {
			Promise.all(promises).then(_ => {
				component_helper.address.setAddressIfComponentExists(e_robot, options, req.body).then(_ => {
					status_helper.setInitialStatus(req.user, e_robot, 'E_robot', attributes).then((statusToastrs = []) => {
...
router.get('/update_form', block_access.actionAccessMiddleware("robot", "update"), (req, res) => {
	const id_e_robot = req.query.id;
	const data = {
		enum_radio: enums_radios.translated("e_robot", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_robot', id_e_robot, options).then(e_robot => {
		...
		e_robot.dataValues.enum_radio = data.enum_radio;
		enums_radios.translateUsingField(e_robot, options, data.enum_radio);
		data.e_robot = e_robot;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_robot, "e_robot", false).then(_ => {
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(req.query.ajax ? e_robot.dataValues : data, options).then(data => {
				if (req.query.ajax) {
					e_robot.dataValues = data;
					res.render('e_robot/update_fields', e_robot.get({
						plain: true
					}));
				} else
					res.render('e_robot/update', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_robot");
			})
		}).catch(err => {
...
router.post('/update', block_access.actionAccessMiddleware("robot", "update"), (req, res) => {
	const id_e_robot = parseInt(req.body.id);
	...
	models.E_robot.findOne({
		...
		component_helper.address.updateAddressIfComponentExists(e_robot, options, req.body);

		updateObject.version = e_robot.version;
		if(typeof e_robot.version === 'undefined' || !e_robot.version)
			updateObject.version = 0;
		updateObject.version++;

		e_robot.update(updateObject, {user: req.user}).then(_ => {

			// We have to find value in req.body that are linked to an hasMany or belongsToMany association
			// because those values are not updated for now
			model_builder.setAssocationManyValues(e_robot, req.body, updateObject, options).then(_ => {

				let redirect = '/robot/show?id=' + id_e_robot;
				if (typeof req.body.associationFlag !== 'undefined')
					redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
				...
				entity_helper.error(err, req, res, '/robot/update_form?id=' + id_e_robot, "e_robot");
			});
		}).catch(err => {
			entity_helper.error(err, req, res, '/robot/update_form?id=' + id_e_robot, "e_robot");
		});
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('robot', 'read'), (req, res) => {
	...
	(async () => {
		...
		switch (option.structureType) {
			case 'hasOne':
				e_robot = await models.E_robot.findOne({
					where: {
						id: req.params.id
					},
					include: {
...
router.get('/set_status/:id_robot/:status/:id_new_status', block_access.actionAccessMiddleware("robot", "read"), block_access.statusGroupAccess, (req, res) => {
	status_helper.setStatus('e_robot', req.params.id_robot, req.params.status, req.params.id_new_status, req.user, req.query.comment).then(_ => {
...
router.post('/search', block_access.actionAccessMiddleware('robot', 'read'), (req, res) => {
	...
	models.E_robot.findAndCountAll(query).then(results => {
					...
					if(fieldSelect == field && results.rows[i][field] && results.rows[i][field] != "")
						switch(attributes[field].newmipsType) {
							...
							case "datetime":
								...
								break;
							case "enum":
								results.rows[i][field] = enums_radios.translateFieldValue('e_robot', field, results.rows[i][field], req.session.lang_user)
								break;
							default:
								break;
						}
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("robot", "update"), (req, res) => {
	const alias = req.params.alias;
	...
	models.E_robot.findOne({
		...
		e_robot['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(_ => {
			if(globalConfig.env == "tablet"){
				let target = "";
				for (let i = 0; i < options.length; i++)
					if (options[i].as == alias)
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("robot", "create"), (req, res) => {
	const alias = req.params.alias;
	const idEntity = req.body.idEntity;
	models.E_robot.findOne({
		where: {
		...
		if (typeof(toAdd = req.body.ids) === 'undefined') {
			...
			return res.redirect('/robot/show?id=' + idEntity + "#" + alias);
		}

		e_robot['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(_ => {
			res.redirect('/robot/show?id=' + idEntity + "#" + alias);
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_robot");
		});
	}).catch(err => {
...
router.post('/delete', block_access.actionAccessMiddleware("robot", "delete"), (req, res) => {
	const id_e_robot = parseInt(req.body.id);
	...
	models.E_robot.findOne({
		...
		deleteObject.destroy().then(_ => {
			...
			let redirect = '/robot/list';
			if (typeof req.body.associationFlag !== 'undefined')
				redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
			res.redirect(redirect);
			entity_helper.removeFiles("e_robot", deleteObject, attributes);
		}).catch(err => {
			entity_helper.error(err, req, res, '/robot/list', "e_robot");
		});
	}).catch(err => {
		entity_helper.error(err, req, res, '/robot/list', "e_robot");
	});
