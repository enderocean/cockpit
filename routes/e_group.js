...
const attributes = require('../models/attributes/e_group');
const options = require('../models/options/e_group');
...
router.get('/list', block_access.actionAccessMiddleware("group", "read"), (req, res) => {
	res.render('e_group/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("group", "read"), (req, res) => {
	filterDataTable("E_group", req.body).then(rawData => {
		entity_helper.prepareDatalistResult('e_group', rawData, req.session.lang_user).then(preparedData => {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("group", "read"), (req, res) => {
	...
	models.E_group.findOne({
		...
		e_group['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]({where: include.where, include: include.include}).then(count => {
			const rawData = {
				recordsTotal: count,
				recordsFiltered: count,
				data: []
			};
			for (let i = 0; i < e_group[subentityAlias].length; i++)
...
router.get('/show', block_access.actionAccessMiddleware("group", "read"), (req, res) => {
	const id_e_group = req.query.id;
	...
	entity_helper.optimizedFindOne('E_group', id_e_group, options).then(e_group => {
		if (!e_group)
		...
		entity_helper.getPicturesBuffers(e_group, "e_group").then(_ => {
			status_helper.translate(e_group, attributes, req.session.lang_user);
			data.componentAddressConfig = component_helper.address.getMapsConfigIfComponentAddressExists("e_group");
			enums_radios.translateUsingField(e_group, options, data.enum_radio);
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(data, options).then(data => {
				res.render('e_group/show', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_group");
			})
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_group");
		});
...
router.get('/create_form', block_access.actionAccessMiddleware("group", "create"), (req, res) => {
	const data = {
		enum_radio: enums_radios.translated("e_group", req.session.lang_user, options)
	};
	...
	// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
	entity_helper.getLoadOnStartData(data, options).then(data => {
		const view = req.query.ajax ? 'e_group/create_fields' : 'e_group/create';
		res.render(view, data);
	}).catch(err => {
		entity_helper.error(err, req, res, '/group/create_form', "e_group");
	})
});

router.post('/create', block_access.actionAccessMiddleware("group", "create"), (req, res) => {

	const createObject = model_builder.buildForRoute(attributes, options, req.body);

	models.E_group.create(createObject, {user: req.user}).then(e_group => {
		let redirect = '/group/show?id=' + e_group.id;
		req.session.toastr = [{
			...
			promises.push(new Promise((resolve, reject) => {
				models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
					...
					if (typeof association['add' + modelName] !== 'undefined') {
						association['add' + modelName](e_group.id).then(_ => {
		...
		model_builder.setAssocationManyValues(e_group, req.body, createObject, options).then(_ => {
			Promise.all(promises).then(_ => {
				component_helper.address.setAddressIfComponentExists(e_group, options, req.body).then(_ => {
					status_helper.setInitialStatus(req.user, e_group, 'E_group', attributes).then((statusToastrs = []) => {
...
router.get('/update_form', block_access.actionAccessMiddleware("group", "update"), (req, res) => {
	const id_e_group = req.query.id;
	const data = {
		enum_radio: enums_radios.translated("e_group", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_group', id_e_group, options).then(e_group => {
		...
		data.e_group = e_group;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_group, "e_group", false).then(_ => {
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(req.query.ajax ? e_group.dataValues : data, options).then(data => {
				if (req.query.ajax) {
					e_group.dataValues = data;
					res.render('e_group/update_fields', e_group.get({
						plain: true
					}));
				} else
...
router.post('/update', block_access.actionAccessMiddleware("group", "update"), (req, res) => {
	const id_e_group = parseInt(req.body.id);
	...
	models.E_group.findOne({
		...
		component_helper.address.updateAddressIfComponentExists(e_group, options, req.body);

		updateObject.version = e_group.version;
		if(typeof e_group.version === 'undefined' || !e_group.version)
			updateObject.version = 0;
		updateObject.version++;

		e_group.update(updateObject, {user: req.user}).then(_ => {

			// We have to find value in req.body that are linked to an hasMany or belongsToMany association
			// because those values are not updated for now
			model_builder.setAssocationManyValues(e_group, req.body, updateObject, options).then(_ => {

				let redirect = '/group/show?id=' + id_e_group;
				if (typeof req.body.associationFlag !== 'undefined')
					redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
				...
				entity_helper.error(err, req, res, '/group/update_form?id=' + id_e_group, "e_group");
			});
		}).catch(err => {
			entity_helper.error(err, req, res, '/group/update_form?id=' + id_e_group, "e_group");
		});
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('group', 'read'), (req, res) => {
	...
	(async () => {
		...
		switch (option.structureType) {
			case 'hasOne':
				e_group = await models.E_group.findOne({
					where: {
						id: req.params.id
					},
					include: {
...
router.get('/set_status/:id_group/:status/:id_new_status', block_access.actionAccessMiddleware("group", "read"), block_access.statusGroupAccess, (req, res) => {
	status_helper.setStatus('e_group', req.params.id_group, req.params.status, req.params.id_new_status, req.user, req.query.comment).then(_ => {
...
router.post('/search', block_access.actionAccessMiddleware('group', 'read'), (req, res) => {
	...
	models.E_group.findAndCountAll(query).then(results => {
					...
					if(fieldSelect == field && results.rows[i][field] && results.rows[i][field] != "")
						switch(attributes[field].newmipsType) {
							...
							case "datetime":
								...
								break;
							case "enum":
								results.rows[i][field] = enums_radios.translateFieldValue('e_group', field, results.rows[i][field], req.session.lang_user)
								break;
							default:
								break;
						}
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("group", "update"), (req, res) => {
	const alias = req.params.alias;
	...
	models.E_group.findOne({
		...
		e_group['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(_ => {
			if(globalConfig.env == "tablet"){
				let target = "";
				for (let i = 0; i < options.length; i++)
					if (options[i].as == alias)
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("group", "create"), (req, res) => {
	const alias = req.params.alias;
	const idEntity = req.body.idEntity;
	models.E_group.findOne({
		where: {
		...
		e_group['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(_ => {
...
router.post('/delete', block_access.actionAccessMiddleware("group", "delete"), (req, res) => {
	const id_e_group = parseInt(req.body.id);
	...
	models.E_group.findOne({
		...
		deleteObject.destroy().then(_ => {
			...
			let redirect = '/group/list';
			if (typeof req.body.associationFlag !== 'undefined')
				redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
			res.redirect(redirect);
			entity_helper.removeFiles("e_group", deleteObject, attributes);
		}).catch(err => {
			entity_helper.error(err, req, res, '/group/list', "e_group");
		});
	}).catch(err => {
		entity_helper.error(err, req, res, '/group/list', "e_group");
	});
