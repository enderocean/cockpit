...
router.get('/list', block_access.actionAccessMiddleware("synchro_credentials", "read"), (req, res) => {
	res.render('e_synchro_credentials/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("synchro_credentials", "read"), (req, res) => {
	filterDataTable("E_synchro_credentials", req.body).then(rawData => {
		entity_helper.prepareDatalistResult('e_synchro_credentials', rawData, req.session.lang_user).then(preparedData => {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("synchro_credentials", "read"), (req, res) => {
	...
	models.E_synchro_credentials.findOne({
		...
		include: include
	}).then(e_synchro_credentials => {
		if (!e_synchro_credentials['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
		...
		e_synchro_credentials['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]({where: include.where, include: include.include}).then(count => {
			const rawData = {
				recordsTotal: count,
				recordsFiltered: count,
				data: []
			};
			for (let i = 0; i < e_synchro_credentials[subentityAlias].length; i++)
				rawData.data.push(e_synchro_credentials[subentityAlias][i].get({plain: true}));
...
router.get('/show', block_access.actionAccessMiddleware("synchro_credentials", "read"), (req, res) => {
	const id_e_synchro_credentials = req.query.id;
	const tab = req.query.tab;
	const data = {
		tab: tab,
		enum_radio: enums_radios.translated("e_synchro_credentials", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_synchro_credentials', id_e_synchro_credentials, options).then(e_synchro_credentials => {
		if (!e_synchro_credentials)
			return res.render('common/error', {error: 404});

		/* Update local e_synchro_credentials data before show */
		data.e_synchro_credentials = e_synchro_credentials;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_synchro_credentials, "e_synchro_credentials").then(_ => {
			status_helper.translate(e_synchro_credentials, attributes, req.session.lang_user);
			data.componentAddressConfig = component_helper.address.getMapsConfigIfComponentAddressExists("e_synchro_credentials");
			enums_radios.translateUsingField(e_synchro_credentials, options, data.enum_radio);
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(data, options).then(data => {
				res.render('e_synchro_credentials/show', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_synchro_credentials");
			})
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_synchro_credentials");
		});
...
router.get('/create_form', block_access.actionAccessMiddleware("synchro_credentials", "create"), (req, res) => {
	const data = {
		enum_radio: enums_radios.translated("e_synchro_credentials", req.session.lang_user, options)
	};
	...
	// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
	entity_helper.getLoadOnStartData(data, options).then(data => {
		const view = req.query.ajax ? 'e_synchro_credentials/create_fields' : 'e_synchro_credentials/create';
		res.render(view, data);
	}).catch(err => {
		entity_helper.error(err, req, res, '/synchro_credentials/create_form', "e_synchro_credentials");
	})
});

router.post('/create', block_access.actionAccessMiddleware("synchro_credentials", "create"), (req, res) => {

	const createObject = model_builder.buildForRoute(attributes, options, req.body);

	models.E_synchro_credentials.create(createObject, {user: req.user}).then(e_synchro_credentials => {
		let redirect = '/synchro_credentials/show?id=' + e_synchro_credentials.id;
		req.session.toastr = [{
			...
			promises.push(new Promise((resolve, reject) => {
				models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
					...
					if (typeof association['add' + modelName] !== 'undefined') {
						...
						const obj = {};
						obj[req.body.associationForeignKey] = e_synchro_credentials.id;
		...
		model_builder.setAssocationManyValues(e_synchro_credentials, req.body, createObject, options).then(_ => {
			Promise.all(promises).then(_ => {
				component_helper.address.setAddressIfComponentExists(e_synchro_credentials, options, req.body).then(_ => {
					status_helper.setInitialStatus(req.user, e_synchro_credentials, 'E_synchro_credentials', attributes).then((statusToastrs = []) => {
...
router.get('/update_form', block_access.actionAccessMiddleware("synchro_credentials", "update"), (req, res) => {
	const id_e_synchro_credentials = req.query.id;
	const data = {
		enum_radio: enums_radios.translated("e_synchro_credentials", req.session.lang_user, options)
	};
	...
	entity_helper.optimizedFindOne('E_synchro_credentials', id_e_synchro_credentials, options).then(e_synchro_credentials => {
		...
		e_synchro_credentials.dataValues.enum_radio = data.enum_radio;
		enums_radios.translateUsingField(e_synchro_credentials, options, data.enum_radio);
		data.e_synchro_credentials = e_synchro_credentials;
		// Update some data before show, e.g get picture binary
		entity_helper.getPicturesBuffers(e_synchro_credentials, "e_synchro_credentials", false).then(_ => {
			// Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
			entity_helper.getLoadOnStartData(req.query.ajax ? e_synchro_credentials.dataValues : data, options).then(data => {
				if (req.query.ajax) {
					e_synchro_credentials.dataValues = data;
					res.render('e_synchro_credentials/update_fields', e_synchro_credentials.get({
						plain: true
					}));
				} else
					res.render('e_synchro_credentials/update', data);
			}).catch(err => {
				entity_helper.error(err, req, res, "/", "e_synchro_credentials");
			})
		}).catch(err => {
...
router.post('/update', block_access.actionAccessMiddleware("synchro_credentials", "update"), (req, res) => {
	const id_e_synchro_credentials = parseInt(req.body.id);
	const updateObject = model_builder.buildForRoute(attributes, options, req.body);

	models.E_synchro_credentials.findOne({
		where: {
			id: id_e_synchro_credentials
		}
	}).then(e_synchro_credentials => {
		if (!e_synchro_credentials)
			return res.render('common/error', {error: 404});

		component_helper.address.updateAddressIfComponentExists(e_synchro_credentials, options, req.body);

		updateObject.version = e_synchro_credentials.version;
		if(typeof e_synchro_credentials.version === 'undefined' || !e_synchro_credentials.version)
			updateObject.version = 0;
		updateObject.version++;

		e_synchro_credentials.update(updateObject, {user: req.user}).then(_ => {

			// We have to find value in req.body that are linked to an hasMany or belongsToMany association
			// because those values are not updated for now
			model_builder.setAssocationManyValues(e_synchro_credentials, req.body, updateObject, options).then(_ => {

				let redirect = '/synchro_credentials/show?id=' + id_e_synchro_credentials;
				if (typeof req.body.associationFlag !== 'undefined')
					redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
				...
				entity_helper.error(err, req, res, '/synchro_credentials/update_form?id=' + id_e_synchro_credentials, "e_synchro_credentials");
			});
		}).catch(err => {
			entity_helper.error(err, req, res, '/synchro_credentials/update_form?id=' + id_e_synchro_credentials, "e_synchro_credentials");
		});
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('synchro_credentials', 'read'), (req, res) => {
	...
	(async () => {
		...
		let e_synchro_credentials;
		switch (option.structureType) {
			case 'hasOne':
				e_synchro_credentials = await models.E_synchro_credentials.findOne({
					where: {
						id: req.params.id
					},
					include: {
				...
				dustData = e_synchro_credentials[option.as];
...
router.get('/set_status/:id_synchro_credentials/:status/:id_new_status', block_access.actionAccessMiddleware("synchro_credentials", "read"), block_access.statusGroupAccess, (req, res) => {
	status_helper.setStatus('e_synchro_credentials', req.params.id_synchro_credentials, req.params.status, req.params.id_new_status, req.user, req.query.comment).then(_ => {
...
router.post('/search', block_access.actionAccessMiddleware('synchro_credentials', 'read'), (req, res) => {
	...
	models.E_synchro_credentials.findAndCountAll(query).then(results => {
					...
					if(fieldSelect == field && results.rows[i][field] && results.rows[i][field] != "")
						switch(attributes[field].newmipsType) {
							...
							case "datetime":
								...
								break;
							case "enum":
								results.rows[i][field] = enums_radios.translateFieldValue('e_synchro_credentials', field, results.rows[i][field], req.session.lang_user)
								break;
							default:
								break;
						}
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("synchro_credentials", "update"), (req, res) => {
	const alias = req.params.alias;
	...
	models.E_synchro_credentials.findOne({
		...
		e_synchro_credentials['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(_ => {
			if(globalConfig.env == "tablet"){
				let target = "";
				for (let i = 0; i < options.length; i++)
					if (options[i].as == alias)
				...
				entity_helper.synchro.writeJournal({
					...
					id: idEntity,
					target: target,
					entityName: "e_synchro_credentials",
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("synchro_credentials", "create"), (req, res) => {
	const alias = req.params.alias;
	const idEntity = req.body.idEntity;
	models.E_synchro_credentials.findOne({
		where: {
		...
		if (typeof(toAdd = req.body.ids) === 'undefined') {
			...
			return res.redirect('/synchro_credentials/show?id=' + idEntity + "#" + alias);
		}

		e_synchro_credentials['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(_ => {
			res.redirect('/synchro_credentials/show?id=' + idEntity + "#" + alias);
		}).catch(err => {
			entity_helper.error(err, req, res, "/", "e_synchro_credentials");
		});
	}).catch(err => {
...
router.post('/delete', block_access.actionAccessMiddleware("synchro_credentials", "delete"), (req, res) => {
	const id_e_synchro_credentials = parseInt(req.body.id);
	...
	models.E_synchro_credentials.findOne({
		...
		deleteObject.destroy().then(_ => {
			...
			let redirect = '/synchro_credentials/list';
			if (typeof req.body.associationFlag !== 'undefined')
				redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
			res.redirect(redirect);
			entity_helper.removeFiles("e_synchro_credentials", deleteObject, attributes);
		}).catch(err => {
			entity_helper.error(err, req, res, '/synchro_credentials/list', "e_synchro_credentials");
		});
	}).catch(err => {
		entity_helper.error(err, req, res, '/synchro_credentials/list', "e_synchro_credentials");
	});
